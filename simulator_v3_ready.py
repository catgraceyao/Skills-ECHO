"""
ECHO Simulator v3 - Corrected Baseline (loss=0.5%) + D as deriveFee
Ready for D=0/1/2 comparison when D definition is confirmed

Key changes from v2:
- Matthew loss: 0.5% (was 1%, now gives target 0.15-0.25)
- D modeled as deriveFee multiplier on new node creation
- D=0: no deriveFee, new node keeps full potential
- D=1: standard deriveFee (1% of new node potential)
- D=2: doubled deriveFee (2% of new node potential)
"""
...
BASE_INCOME_T1 = 1.0
BASE_INCOME_T2 = 0.5
DERIVE_FEE_RATE = 0.01  # 1% base derive fee

MATTHEW_LOSS_PCT = 0.005  # 0.5% loss for bottom 30% (parameter sweep: gives ~0.20 Matthew)
MATTHEW_GAIN_PCT = 0.01  # 1% gain for top 10% (has minimal effect on convergence)
...
class ShiGraphSimulatorV3:
    def __init__(self, d_coefficient=1, seed=42):
        self.d = d_coefficient
        self.derive_fee_multiplier = d_coefficient  # D=0,1,2
        self.seed = seed
        random.seed(seed)
        self.nodes = {}
        self.next_node_id = 0
    ...
    def _add_new_nodes(self):
        ...
        for _ in range(NEW_NODES_PER_ROUND):
            ...
            # DeriveFee: parent node receives fee, new node loses potential
            if self.derive_fee_multiplier > 0 and alive_nodes:
                parent = random.choice(alive_nodes)
                derive_fee = node.potential * DERIVE_FEE_RATE * self.derive_fee_multiplier
                parent.cumulative_income += derive_fee
                node.potential -= derive_fee
...
if __name__ == '__main__':
    ...
    for d in [0, 1, 2]:
        print(f"\n{'='*60}")
        print(f"D={d} (deriveFee multiplier={d}x)")
        print(f"{'='*60}")
        
        sim = ShiGraphSimulatorV3(d_coefficient=d, seed=42)
        result = sim.run()
    ...
    with open('/root/.openclaw/workspace/d_test_v3_results.json', 'w') as f:
    ...
    for key, result in results.items():
        ...
        print(f"D={d}: {matthew:.4f} {'✅' if in_target else '❌'}")
    
    print(f"\nD sensitivity: {results['D_0']['matthew_convergence'] - results['D_2']['matthew_convergence']:.4f}")
