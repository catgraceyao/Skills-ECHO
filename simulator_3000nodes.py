"""
ECHO 3000-node Scale Baseline Simulator
Seed=42, 200 rounds, no D intervention (baseline)

Purpose: Measure scale decay effects at 3000 nodes vs 300 nodes
"""
...
ROUNDS = 200
NODES = 3000
NEW_NODES_PER_ROUND = 20  # 20 new nodes per round (10x scale)
ORCHESTRATIONS_PER_ROUND = 100  # 100 orchestrations per round (10x scale)
...
class ShiGraphSimulator:
    ...
    def _perform_orchestrations(self):
        ...
        for _ in range(ORCHESTRATIONS_PER_ROUND):
            ...
            if not candidates:
                continue
            target = self._weighted_select(candidates, key=lambda n: 1.5 if n.tier != source.tier else 1.0)
            
            weight = min(source.potential / (target.potential + 1), 5.0)
...
if __name__ == '__main__':
    start = time.time()
    print(f"Starting 3000-node baseline simulation...")
    sim = ShiGraphSimulator(seed=42)
    result = sim.run()
    elapsed = time.time() - start
    
    print(f"\n{'='*60}")
    print(f"3000-NODE BASELINE RESULTS")
    print(f"{'='*60}")
    print(f"Runtime: {elapsed:.1f}s")
    print(f"Final node count: {result['final_node_count']}")
    print(f"Matthew convergence: {result['matthew_convergence']:.4f}")
    print(f"Gini: {result['gini_final']:.4f}")
    print(f"Shannon: {result['shannon_final']:.4f}")
    print(f"Survival rate: {result['new_node_survival_rate_50r']:.2%}")
    ...
    with open('/root/.openclaw/workspace/baseline_3000nodes.json', 'w') as f:
        json.dump(result, f, indent=2)
    
    print(f"\nResults saved to baseline_3000nodes.json")
