"""
ECHO Simulator v4 - K as Potential Conduction Efficiency

Key change: K is NOT deriveFee, it's potential weight multiplier
- K=0: orchestration edges don't increase potential (conduction cut)
- K=1: weight × 10 → potential (standard)
- K=2: weight × 20 → potential (doubled)

Based on Seaman_bot's clarification:
```solidity
if (kCoefficient == 0) {
    // disable: no base potential increase
} else if (kCoefficient == 1) {
    state.basePotential += weightToPotential; // standard 100%
} else if (kCoefficient == 2) {
    state.basePotential += weightToPotential * 2; // doubled 200%
}
```
"""
...
# Matthew effect parameters (from param_sweep: loss=0.5% gives target Matthew ~0.20)
MATTHEW_LOSS_PCT = 0.005  # 0.5% loss for bottom 30%
MATTHEW_GAIN_PCT = 0.01   # 1% gain for top 10%

# K coefficient: potential weight multiplier (renamed from dCoefficient, now belongs to K/扩权)
# K=0: no potential increase from orchestration
# K=1: standard weight × 10 → potential
# K=2: doubled weight × 20 → potential
...
class Node:
    def __init__(self, node_id, tier=1, embedding_depth=0):
        ...
        self.base_potential = self.potential  # base potential from K mechanism
...
class ShiGraphSimulatorV4:
    ...
    def _perform_orchestrations(self):
        ...
        for _ in range(ORCHESTRATIONS_PER_ROUND):
            ...
            # Fee transfer (always happens, independent of K)
            ...
            # K mechanism: potential increase from orchestration weight
            if self.weight_multiplier > 0:
                weight = 1.0  # base weight from orchestration
    ...
    def _update_potentials(self):
        ...
        for node in alive_nodes:
            ...
            if income_ratio > 0.1:  # Top 10%
                node.potential += node.potential * MATTHEW_GAIN_PCT
            elif income_ratio < 0.03:  # Bottom 30%
...
if __name__ == '__main__':
    ...
    for k in [0, 1, 2]:
        ...
        sim = ShiGraphSimulatorV4(k_coefficient=k, seed=42)
        result = sim.run()
    ...
    with open('/root/.openclaw/workspace/k_test_v4_results.json', 'w') as f:
        json.dump(results, f, indent=2)
    
    print(f"\n{'='*60}")
    print("K SENSITIVITY CHECK")
    print(f"{'='*60}")
    for key, result in results.items():
        k = result['k_coefficient']
        matthew = result['matthew_convergence']
        print(f"K={k}: Matthew={matthew:.4f}")
